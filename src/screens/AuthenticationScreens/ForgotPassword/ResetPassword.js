...
import { RESET_PASSWORD_URL } from "../../../services/ApiConstants";
...
const ResetPassword = ({ route, navigation }) => {
  ...
  const hanldeChangePassword = async () => {
    ...
    try {
      let response = await postRequest(RESET_PASSWORD_URL, sendingData);
  ...
  return (
    <SafeAreaView style={{ backgroundColor: COLORS.white, flex: 1 }}>
      ...
      <View style={styles.viewBtn}>
        <CustomButton title={t("save")} onPress={validate} loading={loading} />
      </View>
    </SafeAreaView>
  );
};
